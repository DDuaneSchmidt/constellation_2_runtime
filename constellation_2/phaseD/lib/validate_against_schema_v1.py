"""
validate_against_schema_v1.py

Constellation 2.0 Phase D
jsonschema validation helper (OFFLINE-SAFE, used at broker boundary).

Fail-closed rules:
- Missing schema file => HARD FAIL (raise)
- Invalid schema JSON => HARD FAIL (raise)
- Invalid instance JSON object => HARD FAIL (raise)
- Floats are forbidden anywhere in the instance (determinism standard)

Schema standard:
- JSON Schema draft 2020-12 (matches schema files)

This module does NOT read from network. Schemas are loaded from repo files only.
"""
