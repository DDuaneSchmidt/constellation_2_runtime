"""
ib_payload_stock_order_v1.py

Constellation 2.0 Phase D
Deterministic Interactive Brokers STK order payload digest builder.

Purpose:
- Build a deterministic broker payload object for equity spot orders
- Compute digest (SHA-256) over canonical JSON bytes
- Used for BindingRecord v2 (broker_payload_digest.format == IB_STK_ORDER_V1)

Hard rules:
- Supports ONLY EquityOrderPlan v1 (schema_id=equity_order_plan, schema_version=v1)
...
def build_ib_stk_order_payload_v1(equity_order_plan: Dict[str, Any]) -> Dict[str, Any]:
    if not isinstance(equity_order_plan, dict):
        raise IBPayloadError("EQUITY_ORDER_PLAN_NOT_OBJECT")

    if equity_order_plan.get("schema_id") != "equity_order_plan" or equity_order_plan.get("schema_version") != "v1":
...
def build_binding_digest_for_equity_order_plan_v1(equity_order_plan: Dict[str, Any]) -> Tuple[Dict[str, Any], IBPayloadDigestV1]:
    payload = build_ib_stk_order_payload_v1(equity_order_plan)
