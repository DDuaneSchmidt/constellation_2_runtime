...
from constellation_2.phaseF.positions.lib.paths_v5 import day_paths_v5
...
def _choose_snapshot(day_utc: str) -> Tuple[str, int, Path]:
    # Prefer v5 if present; else v4; else v3; else v2.
    p5 = day_paths_v5(day_utc).snapshot_path
...
def main(argv: List[str] | None = None) -> int:
    ap = argparse.ArgumentParser(
        prog="run_positions_effective_pointer_day_v2",
        description="C2 Positions Effective Pointer v2 (select best available positions snapshot for a day; prefers v5).",
    )
    ...
    try:
        ...
        failure = build_failure_obj_v1(
            ...
            producer_module="constellation_2/phaseF/positions/run/run_positions_effective_pointer_day_v2.py",
            status="FAIL_CORRUPT_INPUTS",
            reason_codes=["NO_POSITIONS_SNAPSHOT_FOUND"],
            input_manifest=[],
            code="FAIL_CORRUPT_INPUTS",
            message="No positions snapshot found for day (need v5, v4, v3, or v2 snapshot).",
    ...
    print("OK: POSITIONS_EFFECTIVE_POINTER_V2_WRITTEN")
