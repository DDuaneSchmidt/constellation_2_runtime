...
SCHEMA_SUBMISSION_MANIFEST = "governance/04_DATA/SCHEMAS/C2/EXECUTION_EVIDENCE/execution_evidence_submission_manifest.v1.schema.json"
...
def main(argv: List[str] | None = None) -> int:
    ...
    # Scan submission dirs deterministically.
    ...
    for sd in sub_dirs:
        ...
        # Validate PhaseD records against their native schemas when present.
        ...
        if derived_day != day_utc:
            continue  # not this day

        if broker_obj is None:
            # Current manifest schema requires broker_submission_record; skip veto-only dirs in v1.
        ...
        # Write canonical truth mirror.
        art_dir = submission_artifact_dir_v1(day_utc=day_utc, submission_id=submission_id)
        art_dir.mkdir(parents=True, exist_ok=True)

        # Copy bytes immutably (use source bytes directly; already schema-valid JSON).
        ...
        # Build submission manifest (governed schema).
        input_manifest = [
            {"type": "phaseD_submission_dir", "path": str(sd), "sha256": "0"*64, "day_utc": day_utc, "producer": "phaseD"}
        ]
        manifest_obj: Dict[str, Any] = {
            "schema_id": "C2_EXECUTION_EVIDENCE_SUBMISSION_MANIFEST_V1",
            "schema_version": 1,
        ...
        validate_against_repo_schema_v1(manifest_obj, REPO_ROOT, SCHEMA_SUBMISSION_MANIFEST)
        try:
    ...
    # Build latest pointer (pointer-only).
    summary = {
        "day_utc": day_utc,
