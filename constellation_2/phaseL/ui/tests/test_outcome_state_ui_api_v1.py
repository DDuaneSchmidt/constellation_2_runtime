...
from constellation_2.phaseL.ui_api.outcome_state_read_model import build_outcome_state_view


def test_outcome_view_stays_missing_without_value_artifact_even_when_upstream_truth_exists(
    ...
    monkeypatch.setattr("constellation_2.phaseL.ui_api.outcome_state_read_model.GLOBAL_TRUTH_ROOT", canonical_truth)
    payload = build_outcome_state_view(DAY)
    assert payload["view_name"] == "outcomes"
    assert payload["outcome_rows"] == []
    assert payload["outcome_warnings"] == ["VALUE_STATE_ARTIFACT_MISSING"]


def test_outcome_view_renders_governed_value_artifacts_only(
    monkeypatch: pytest.MonkeyPatch,
    tmp_path: Path,
) -> None:
    ...
    monkeypatch.setattr("constellation_2.phaseL.ui_api.outcome_state_read_model.GLOBAL_TRUTH_ROOT", canonical_truth)
    payload = build_outcome_state_view(DAY)
    assert payload["outcome_warnings"] == []
    assert payload["outcome_rows"]
    assert payload["source_refs"][0]["artifact_type"] == "value_state_v1"


def test_outcome_route_and_page_are_artifact_backed() -> None:
