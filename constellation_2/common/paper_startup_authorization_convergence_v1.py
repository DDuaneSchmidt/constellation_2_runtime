...
SCHEMA_RELPATH = "governance/04_DATA/SCHEMAS/C2/REPORTS/paper_startup_authorization_convergence.v1.schema.json"


def resolve_paper_startup_authorization_convergence_path(*, truth_root: Path, day_utc: str) -> Path:
    return (
...
def derive_paper_startup_authorization_convergence_payload_v1(
    *,
    truth_root: Path,
    target_day: str,
    environment: str,
    ...
    verdict_ready = any(
        str(row.get("artifact_id") or "").strip() == "authorization_gate_verdict_v1"
        and bool(row.get("ready") is True)
        for row in artifact_results
    )
    convergence_status = "SUCCESS" if verdict_ready and not blocker_rows else "BLOCKED"
    required_artifacts = [
    ...
    return {
        ...
        "authorization_verdict_ready": bool(verdict_ready),
        "authorization_required_artifacts": required_artifacts,
...
def write_paper_startup_authorization_convergence_v1(*, truth_root: Path, payload: Dict[str, Any]) -> SurfaceRefV1:
    return atomic_write_idempotent_validated_json_v1(
        path=resolve_paper_startup_authorization_convergence_path(
