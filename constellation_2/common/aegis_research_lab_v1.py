...
SCHEMA_RELPATHS = {
    ...
    "evidence_gate": "governance/04_DATA/SCHEMAS/C2/REPORTS/evidence_gate.v1.schema.json",
    "ai_feedback_review": "governance/04_DATA/SCHEMAS/C2/REPORTS/ai_feedback_review.v1.schema.json",
    "eod_sleeve_review": "governance/04_DATA/SCHEMAS/C2/REPORTS/eod_sleeve_review.v1.schema.json",
    "eow_sleeve_review": "governance/04_DATA/SCHEMAS/C2/REPORTS/eow_sleeve_review.v1.schema.json",
...
def build_manual_execution_receipt_v1(
    *,
    ...
    quantity_override = int(suggested_quantity or 0) > 0 and int(actual_quantity) != int(suggested_quantity or 0)
    payload = {
        ...
        "expected_risk": str(expected_risk or ""),
        "actual_risk": str(actual_risk or ""),
        "operator_override_reason": str(operator_override_reason or ""),
        "sizing_quality": str(sizing_quality or ("OVERRIDDEN" if quantity_override else "MATCHED" if int(suggested_quantity or 0) > 0 else "UNKNOWN")),
...
def _outcome_row(row: dict[str, Any]) -> dict[str, Any]:
    return {
        ...
        "suggested_quantity": int(row.get("suggested_quantity") or 0),
        "actual_quantity": int(row.get("actual_quantity") or 0),
        "expected_risk": str(row.get("expected_risk") or ""),
        "actual_risk": str(row.get("actual_risk") or ""),
        "operator_override_reason": str(row.get("operator_override_reason") or ""),
        "sizing_quality": str(row.get("sizing_quality") or ""),
