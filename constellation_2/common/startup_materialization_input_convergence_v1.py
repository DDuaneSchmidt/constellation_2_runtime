...
SCHEMA_RELPATH = "governance/04_DATA/SCHEMAS/C2/REPORTS/startup_materialization_input_convergence.v1.schema.json"


def resolve_startup_materialization_input_convergence_path(*, truth_root: Path, day_utc: str) -> Path:
    return (
...
def derive_startup_materialization_input_convergence_payload_v1(
    *,
    truth_root: Path,
    target_day: str,
...
def write_startup_materialization_input_convergence_v1(*, truth_root: Path, payload: Dict[str, Any]) -> SurfaceRefV1:
    return atomic_write_idempotent_validated_json_v1(
        path=resolve_startup_materialization_input_convergence_path(
