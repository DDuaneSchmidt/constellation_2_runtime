...
from constellation_2.common.value_claim_strength_v1 import (
    CLAIM_LADDER_VERSION,
    evaluate_value_claim_strength_v1,
)
from constellation_2.common.value_comparison_v1 import (
    COMPARISON_VERSION,
    evaluate_value_comparison_v1,
)
from constellation_2.common.value_effectiveness_attribution_v1 import (
    MODEL_VERSION as EFFECTIVENESS_MODEL_VERSION,
    evaluate_value_effectiveness_attribution_v1,
)
from constellation_2.common.value_explanation_mapping_v1 import (
    EXPLANATION_MAPPING_VERSION,
    build_value_explanation_v1,
)
from constellation_2.common.value_sleeve_linkage_v1 import (
    LINKAGE_VERSION as SLEEVE_LINKAGE_VERSION,
    evaluate_value_sleeve_linkage_v1,
)
from constellation_2.phaseD.lib.canon_json_v1 import canonical_hash_for_c2_artifact_v1


REPO_ROOT = Path(__file__).resolve().parents[2]
WRITER_ID = "constellation_2.common.value_state_kernel_v1"
KERNEL_VERSION = WRITER_ID
ARTIFACT_ID = "value_state_v1"
SCHEMA_RELPATH = "governance/04_DATA/SCHEMAS/C2/RUNTIME/value_state.v1.schema.json"
...
class ValueStateError(RuntimeError):
    pass
...
def _assert_allowed_truth_root(canonical_truth_root: Path) -> None:
    ...
    if resolved == REPO_ROOT or repo_runtime_root == resolved or repo_runtime_root in resolved.parents:
        raise ValueStateError(f"VALUE_STATE_FORBIDDEN_TRUTH_ROOT:{resolved}")
...
def list_value_states_v1(
    *,
    ...
    if not root.exists():
        return refs
    pattern = "*/*/value_state.v1.json" if not scope_id else f"{scope_id}/*/value_state.v1.json"
    ...
    refs.sort(
        key=lambda ref: (
            ...
            str(ref.payload.get("value_id") or ""),
            str(ref.path),
        )
    )
    return refs
...
def _find_latest_value_for_subject(
    *,
    ...
    refs = [
        ref
        for ref in list_value_states_v1(canonical_truth_root=canonical_truth_root, day_utc=day_utc, scope_id=scope_id)
...
def _write_value(payload: dict[str, Any], scope: Any) -> SurfaceRefV1:
    ...
    output_path = resolve_constitutional_artifact_path_v1(
        ...
        extra_variables={"scope_id": scope.scope_id, "value_id": payload["value_id"]},
    )
...
def materialize_value_state_v1(
    *,
    day_utc: str,
    sleeve_id: str,
    environment: str,
    ...
    if not opportunity_refs:
        raise ValueStateError("VALUE_STATE_REQUIRES_OPPORTUNITY_STATE")
    ...
    if tax_ref is None:
        raise ValueStateError("VALUE_STATE_REQUIRES_TAX_STATE")
    ...
    if product_summary_ref is None:
        raise ValueStateError("VALUE_STATE_REQUIRES_PRODUCT_SUMMARY")
    ...
    if product_snapshot_ref is None:
        raise ValueStateError("VALUE_STATE_REQUIRES_PRODUCT_SNAPSHOT")
    ...
    for opportunity_ref in opportunity_refs:
        ...
        comparison_state = evaluate_value_comparison_v1(
            requested_comparison_type=comparison_type,
            opportunity_payload=opportunity_payload,
            realized_activity_observed=bool(basis["realized_activity_observed"]),
            no_activity_observed=bool(basis["no_activity_observed"]),
        )
        sleeve_linkage = evaluate_value_sleeve_linkage_v1(
            scope_sleeve_id=scope.sleeve_id,
            opportunity_payload=opportunity_payload,
        )
        effectiveness = evaluate_value_effectiveness_attribution_v1(
            opportunity_payload=opportunity_payload,
            realized_state=realized_state,
            observability_state=str(basis["observability_state"]),
            comparison_state=comparison_state,
            sleeve_linkage_state=str(sleeve_linkage["primary_linkage_state"]),
        )
        claim = evaluate_value_claim_strength_v1(
            {
        ...
        value_id = canonical_hash_for_c2_artifact_v1(
            {
                "artifact_id": ARTIFACT_ID,
                "scope_id": scope.scope_id,
                ...
                "sleeve_refs": sleeve_linkage["sleeve_refs"],
                "comparison_state": comparison_state,
            }
        )
        payload = {
            "schema_id": "value_state",
            "schema_version": "v1",
            "artifact_id": ARTIFACT_ID,
            ...
            "authority_label": "governed_certified_value",
            ...
            "sleeve_refs": [dict(row) for row in sleeve_linkage["sleeve_refs"]],
        ...
        payload["primary_explanation"] = build_value_explanation_v1(value_payload=payload)
        payload["semantic_events"] = [
            {
                "event_type": "value_state_computed",
                ...
                "sleeve_refs": payload["sleeve_refs"],
        ...
        if product_snapshot_refs:
            payload["semantic_events"].append(
                {
                    "event_type": "product_value_snapshot_created",
                    "authority_label": payload["authority_label"],
                    "realized_state": payload["realized_state"],
                    "observability_state": payload["observability_state"],
                    "effectiveness_state": payload["effectiveness_state"],
        ...
        prior_ref = _find_latest_value_for_subject(
            canonical_truth_root=scope.canonical_truth_root,
            day_utc=scope.day_utc,
            scope_id=scope.scope_id,
            subject_opportunity_id=str(opportunity_payload.get("opportunity_id") or ""),
        )
        if prior_ref is not None and str(prior_ref.payload.get("value_id") or "") != value_id:
            payload["supersedes_ref"] = _plain_ref_from_surface(prior_ref, ARTIFACT_ID)

        if emit_artifacts:
            artifact_ref = _write_value(payload, scope)
