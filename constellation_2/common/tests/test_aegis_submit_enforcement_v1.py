...
def _packet(runtime_root: Path, commit: str = COMMIT) -> None:
    _write(
...
def _seed_ready(tmp_path: Path) -> tuple[Path, Path, Path]:
...
def _evaluate(truth: Path, execution: Path, runtime: Path) -> dict:
