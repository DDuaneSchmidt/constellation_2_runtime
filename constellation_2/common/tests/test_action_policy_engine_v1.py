...
def test_action_policy_engine_applies_v1_rules() -> None:
    ...
    assert action_types == ['raise_cash_reserve', 'withdraw_from_taxable', 'hold_annuity']
    support = {item.action_type: item.support_status for item in result.actions}
    assert support['raise_cash_reserve'] == 'fully_supported'
    assert support['withdraw_from_taxable'] == 'provisional'
    assert support['hold_annuity'] == 'fully_supported'
    withdrawal = next(item for item in result.actions if item.action_type == 'withdraw_from_taxable')
    assert withdrawal.periodicity == 'monthly'
    assert withdrawal.periodic_amount == 150000
    assert withdrawal.annual_amount == 1800000
    blocked = result.blocked_actions
    assert len(blocked) == 1
    assert blocked[0].action_type == 'collect_missing_input'
    assert blocked[0].support_status == 'blocked'
    assert blocked[0].blockers == ('TAX_PROFILE_MISSING',)
    trigger_types = [item.trigger_type for item in result.replan_triggers]
    assert trigger_types == ['employment_change', 'market_drawdown', 'spending_deviation', 'tax_profile_update']
