...
def test_action_compiler_is_deterministic() -> None:
    snapshot = make_planning_snapshot(tax_profile_present=False)
    recommendation_set = make_recommendation_set()
    policy_pack = make_policy_pack()

    first = compile_action_intent(
        planning_snapshot=snapshot,
        official_recommendation_set=recommendation_set,
        action_policy_pack=policy_pack,
    )
    second = compile_action_intent(
        planning_snapshot=snapshot,
        official_recommendation_set=recommendation_set,
        action_policy_pack=policy_pack,
    )
    ...
    assert [item.support_status for item in first.actions] == ['fully_supported', 'provisional', 'fully_supported']
    assert [item.support_status for item in first.blocked_actions] == ['blocked']


def test_cli_writes_only_requested_output_path(tmp_path: Path) -> None:
    snapshot = make_planning_snapshot(tax_profile_present=False)
    recommendation_set = make_recommendation_set()
    policy_pack = make_policy_pack()
    input_dir = tmp_path / 'inputs'
    input_dir.mkdir(parents=True, exist_ok=True)
    out_path = tmp_path / 'compiled' / 'action_intent.json'
    truth_root = tmp_path / 'constellation_2_foundation' / 'truth_sleeves' / 'TEST' / 'PAPER'

    snapshot_path = input_dir / 'planning_snapshot.json'
    recommendation_path = input_dir / 'official_recommendation_set.json'
    policy_pack_path = input_dir / 'action_policy_pack.json'
    snapshot_path.write_bytes(canonical_json_bytes_v1(snapshot.to_dict()) + b'\n')
    recommendation_path.write_bytes(canonical_json_bytes_v1(recommendation_set.to_dict()) + b'\n')
    policy_pack_path.write_bytes(canonical_json_bytes_v1(policy_pack.to_dict()) + b'\n')

    cmd = [
        sys.executable,
        str(REPO_ROOT / 'ops' / 'tools' / 'run_action_policy_compile_v1.py'),
        '--planning_snapshot_json', str(snapshot_path),
        '--official_recommendation_set_json', str(recommendation_path),
