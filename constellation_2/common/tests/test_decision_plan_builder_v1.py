...
def test_decision_plan_builder_is_reproducible() -> None:
    ...
    assert first.planning_snapshot_id == 'planning_snapshot_001'
    assert first.advisory_packet_id == 'advisory_packet_001'
    assert [item.support_status for item in first.actions] == ['fully_supported', 'provisional', 'fully_supported']
    assert [item.action_type for item in first.blocked_actions] == ['collect_missing_input']
    assert 'policy_pack_id:policy_pack_001' in first.constraints_used
