...
def _manifest_payload(*, release_root: Path) -> dict[str, object]:
    return {
        ...
        "schema_version": "v1",
        "release_id": "release-20260420-live",
        "git_sha": "a" * 40,
    ...
    *,
    ...
    payload = build_release_current_v1(
        release_manifest_ref=_ref(
            ...
            surface="release_manifest_active",
            path=manifest_path,
            payload=_manifest_payload(release_root=Path(str(runtime_payload["release_root"]))),
            schema_relpath="governance/04_DATA/SCHEMAS/C2/RELEASES/release_manifest.v1.schema.json",
        ),
        active_runtime_contract_ref=_ref(
            domain="release",
            surface="active_runtime_contract",
            path=(truth_root.parent / "runtime_contract_v1" / "active_runtime_contract.v1.json").resolve(),
            payload=runtime_payload,
            schema_relpath="governance/04_DATA/SCHEMAS/C2/RUNTIME/active_runtime_contract.v1.schema.json",
        ),
        configuration_state_ref=_ref(
            domain="policy",
            surface="configuration_state_current",
            path=(truth_root / "configuration_state_v1" / "current.json").resolve(),
            payload=configuration_payload,
            schema_relpath="governance/04_DATA/SCHEMAS/C2/RUNTIME/configuration_state.v1.schema.json",
        ),
        generated_at_utc="2026-04-20T14:05:00Z",
        producer_module="test_release_current_live_cutover_v1.py",
    )
    payload["release_current_id"] = "e" * 64
    payload["release_id"] = "release-20260420-live-cutover"
    payload["git_sha"] = "f" * 40
    release_cutover_root = (truth_root.parent / "release_cutover").resolve()
    release_cutover_root.mkdir(parents=True, exist_ok=True)
    payload["release_root"] = str(release_cutover_root)
    return payload


def test_release_current_first_uses_release_current_when_valid_and_logs(
    monkeypatch: pytest.MonkeyPatch,
    tmp_path: Path,
    ...
    _, manifest_path = _install_runtime_contract_fixture(
        monkeypatch,
        tmp_path=tmp_path,
    ...
    write_release_current_v1(truth_root=truth_root, payload=current_payload)

    caplog.set_level("INFO")
    result = resolve_release_provenance_release_current_first_v1(caller="test_cutover")

    assert result["release_id"] == current_payload["release_id"]
    assert result["git_sha"] == current_payload["git_sha"]
    assert result["release_root"] == current_payload["release_root"]
    assert result["release_manifest_path"] == str(manifest_path.resolve())
    assert result["provenance_mode"] == "release_current_v1"
    ...
    assert "release_current_cutover_active" in caplog.text
    assert "test_cutover" in caplog.text
    assert str(current_payload["git_sha"]) in caplog.text


def test_release_current_first_falls_back_to_old_when_missing(
    monkeypatch: pytest.MonkeyPatch,
    tmp_path: Path,
) -> None:
    truth_root = (tmp_path / "truth").resolve()
    ...
    baseline = resolve_release_provenance()
    cutover = resolve_release_provenance_release_current_first_v1(caller="test_missing")

    assert cutover == baseline
    assert set(cutover.keys()) == {
        "release_id",
        "git_sha",
        "release_root",
        "release_manifest_path",
...
def test_release_current_first_falls_back_to_old_when_malformed(
    monkeypatch: pytest.MonkeyPatch,
    tmp_path: Path,
) -> None:
    truth_root = (tmp_path / "truth").resolve()
    ...
    cutover = resolve_release_provenance_release_current_first_v1(caller="test_invalid")

    assert cutover["release_id"] == manifest_payload["release_id"]
    assert cutover["git_sha"] == manifest_payload["git_sha"]
    assert cutover["release_root"] == str(release_root.resolve())
    assert cutover["release_manifest_path"].endswith("release_manifest.v1.json")
    assert cutover["provenance_mode"] == "release_manifest"


def test_only_the_approved_low_risk_callers_use_release_current_first_helper() -> None:
    approved = {
    ...
    for relpath in approved:
        text = (SOURCE_ROOT / relpath).read_text(encoding="utf-8")
        assert "resolve_release_provenance_release_current_first_v1" in text
        assert '.get("git_sha")' in text


def test_runtime_critical_callers_remain_on_old_provenance_helper() -> None:
    do_not_migrate = {
        ...
        "constellation_2/common/runtime_path_authority_v1.py",
    }
    for relpath in do_not_migrate:
        text = (SOURCE_ROOT / relpath).read_text(encoding="utf-8")
        assert "resolve_release_provenance_release_current_first_v1" not in text
