...
from constellation_2.common.value_state_kernel_v1 import (
    ValueStateError,
    list_value_states_v1,
...
def _write_reconciliation_report(canonical_truth: Path, *, submissions_total: int, reason_codes: list[str]) -> Path:
    payload = {
        ...
        "producer": {
            "repo": "constellation",
            "module": "test_value_state_kernel_v1",
            "git_sha": "a" * 40,
        },
        "status": "OK",
...
def test_value_state_is_deterministic_for_same_governed_truth(
    ...
    first = materialize_value_state_v1(
        day_utc=DAY,
        sleeve_id=SLEEVE,
        environment=ENV,
        ib_account=ACCOUNT,
    ...
    second = materialize_value_state_v1(
        day_utc=DAY,
        sleeve_id=SLEEVE,
        environment=ENV,
        ib_account=ACCOUNT,
...
def test_value_claim_strength_withholds_when_realized_basis_missing(
    monkeypatch: pytest.MonkeyPatch,
    tmp_path: Path,
) -> None:
    canonical_truth, sleeve_root, *_ = _seed_product_runtime(monkeypatch, tmp_path)
    _materialize_product_summary(monkeypatch, canonical_truth, sleeve_root)
    report = materialize_value_state_v1(
        day_utc=DAY,
        sleeve_id=SLEEVE,
        environment=ENV,
        ib_account=ACCOUNT,
...
def test_value_effectiveness_sleeve_usefulness_and_bounded_attribution_are_conservative(
    monkeypatch: pytest.MonkeyPatch,
    tmp_path: Path,
) -> None:
    ...
    assert blocked["sleeve_refs"][0]["linkage_state"] == "execution_scope_only"


def test_value_no_speculative_attribution_on_realized_activity(
    monkeypatch: pytest.MonkeyPatch,
    tmp_path: Path,
) -> None:
    canonical_truth, sleeve_root, *_ = _seed_product_runtime(monkeypatch, tmp_path)
...
def test_value_comparison_rejects_unsupported_type(
    monkeypatch: pytest.MonkeyPatch,
    tmp_path: Path,
) -> None:
    canonical_truth, sleeve_root, *_ = _seed_product_runtime(monkeypatch, tmp_path)
    ...
    assert all(row["comparison_state"]["reason_id"] == "VALUE_COMPARISON_UNSUPPORTED_TYPE" for row in report["rows"])


def test_value_explanation_and_lineage_are_reconstructable(
    monkeypatch: pytest.MonkeyPatch,
    tmp_path: Path,
) -> None:
    canonical_truth, sleeve_root, *_ = _seed_product_runtime(monkeypatch, tmp_path, blocked_readiness=True)
    ...
    rows = list_value_states_v1(canonical_truth_root=canonical_truth, day_utc=DAY)
    assert rows
    assert any("primary_explanation" in row.payload for row in rows)
    assert any("sleeve_refs" in row.payload for row in rows)
...
def test_value_fails_closed_without_product_summary(
    monkeypatch: pytest.MonkeyPatch,
    tmp_path: Path,
) -> None:
    canonical_truth, sleeve_root, *_ = _seed_product_runtime(monkeypatch, tmp_path)
    with pytest.raises(ValueStateError, match="VALUE_STATE_REQUIRES_PRODUCT_SUMMARY"):
        materialize_value_state_v1(
            day_utc=DAY,
            sleeve_id=SLEEVE,
            environment=ENV,
...
def test_value_rejects_forbidden_repo_local_truth_root(monkeypatch: pytest.MonkeyPatch, tmp_path: Path) -> None:
    canonical_truth, sleeve_root, *_ = _seed_product_runtime(monkeypatch, tmp_path)
    _materialize_product_summary(monkeypatch, canonical_truth, sleeve_root)
    with pytest.raises(ValueStateError, match="VALUE_STATE_FORBIDDEN_TRUTH_ROOT"):
        materialize_value_state_v1(
            day_utc=DAY,
            sleeve_id=SLEEVE,
            environment=ENV,
