...
def test_raw_candidates_upstream_but_eod_candidate_input_missing_is_fail_loud(tmp_path: Path) -> None:
    ...
    assert report["report_status"] == "INPUT_CONTRACT_FAILED"
    ...
    assert "CANDIDATE_INPUT_MISSING" in report["do_not_trade_blockers"]
    assert "PROMOTED_SLEEVE_LIBRARY_REQUIRED" in report["do_not_trade_blockers"]
    assert report["empty_section_reasons"]["executable_manual_trades"] == "candidates not supplied to EOD"
    ...
    assert manifest["overall_status"] == "INPUT_CONTRACT_FAILED"
    assert "CANDIDATE_INPUT_MISSING" in manifest["blockers"]
...
def test_stale_market_snapshot_blocks_candidates_with_visible_symbols(tmp_path: Path) -> None:
    _write_market_snapshot(tmp_path, "2026-05-14", ["SPY"])
    _write_upstream_candidate_manifest(tmp_path, DAY, [_raw_candidate(1)])
    payload = prepare_operational_input_payload_v1(candidate_input_path="", promoted_sleeve_library_path="", manual_only=True)
