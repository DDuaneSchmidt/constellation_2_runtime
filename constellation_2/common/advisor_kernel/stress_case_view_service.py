...
from constellation_2.common.advisor_kernel.stress_case_view_v1 import StressCaseViewV1


def build_stress_case_view(*, planning_snapshot: PlanningSnapshotV1, metadata: dict[str, Any]) -> StressCaseViewV1:
    obj = build_base('stress_case_view', 'view_authority', metadata, 'stress_case_view_v1')
    obj.update({'support_status': 'provisional', 'planning_snapshot_id': planning_snapshot.planning_snapshot_id, 'scenario_ids': ['base_stub'], 'outcomes': ['no_financial_math'], 'summary': 'stub_only_view'})
    return StressCaseViewV1.from_dict(obj)
