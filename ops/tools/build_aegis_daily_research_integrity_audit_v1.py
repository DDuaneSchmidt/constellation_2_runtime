#!/usr/bin/env python3
from __future__ import annotations

import argparse
import json
import sys
from pathlib import Path

REPO_ROOT = Path(__file__).resolve().parents[2]
if str(REPO_ROOT) not in sys.path:
    sys.path.insert(0, str(REPO_ROOT))

from ops.aegis.daily_research_integrity_audit_v1 import build_daily_research_integrity_audit_v1, write_daily_research_integrity_audit_v1


def main() -> int:
    parser = argparse.ArgumentParser(prog="build_aegis_daily_research_integrity_audit_v1")
    parser.add_argument("--truth-root", "--truth_root", dest="truth_root", default="/home/node/constellation_runtime_data/truth")
    parser.add_argument("--day", "--day-utc", "--day_utc", dest="day_utc", required=True)
    args = parser.parse_args()
    payload = build_daily_research_integrity_audit_v1(truth_root=Path(args.truth_root), day_utc=args.day_utc)
    path = write_daily_research_integrity_audit_v1(truth_root=Path(args.truth_root), day_utc=args.day_utc, payload=payload)
    print(json.dumps({
        "artifact": "aegis_daily_research_integrity_audit_v1",
        "ok": True,
        "path": str(path),
        "integrity_status": payload.get("integrity_status"),
        "primary_issue": payload.get("primary_issue"),
        "issue_count": payload.get("issue_count"),
        "blocker_count": payload.get("blocker_count"),
        "warning_count": payload.get("warning_count"),
    }, sort_keys=True))
    return 0


if __name__ == "__main__":
    raise SystemExit(main())
