...
def main() -> int:
    parser = argparse.ArgumentParser(description="Build hostile review for Aegis Alpha Factory POC V1.")
    parser.add_argument("--truth-root", "--truth_root", dest="truth_root", required=True)
    parser.add_argument("--day", required=True)
    args = parser.parse_args()
    payload = build_alpha_factory_poc_review_v1(truth_root=Path(args.truth_root), day_utc=str(args.day))
    paths = write_alpha_factory_poc_review_v1(truth_root=Path(args.truth_root), day_utc=str(args.day), payload=payload)
    result = {"ok": True, "day_utc": str(args.day), "paths": paths, "verdicts": payload.get("verdicts", {})}
    print("AEGIS ALPHA FACTORY POC REVIEW v1")
