...
def main() -> int:
    parser = argparse.ArgumentParser(description="Build Aegis Alpha Factory Evidence Test v2 artifact.")
    parser.add_argument("--truth-root", required=True)
    parser.add_argument("--day", required=True, dest="day_utc")
    args = parser.parse_args()

    payload = build_alpha_factory_evidence_test_v2(
        truth_root=Path(args.truth_root),
        day_utc=args.day_utc,
    )
    paths = write_alpha_factory_evidence_test_v2(
        truth_root=Path(args.truth_root),
        day_utc=args.day_utc,
        payload=payload,
    )
    print("AEGIS ALPHA FACTORY EVIDENCE TEST v2")
    ...
    return 0 if payload["verdicts"]["execution"] == "EVIDENCE_TEST_V2_EXECUTION_VALID" else 1
