...
from constellation_2.common.advisor_kernel.quarterly_action_summary_service import build_quarterly_action_summary


def main() -> int:
    parser = argparse.ArgumentParser(description='Emit quarterly_action_summary_v1')
    ...
    metadata = _metadata(args, [f'decision_plan_id:{decision_plan.plan_id}'])
    artifact = build_quarterly_action_summary(decision_plan=decision_plan, metadata=metadata)
    out = _write(_runtime_path(args.output_root, args.mode, 'quarterly_action_summary_v1', args.day_utc, 'quarterly_action_summary.v1.json'), artifact.to_dict())
    print(f'OK: {out}')
    return 0
