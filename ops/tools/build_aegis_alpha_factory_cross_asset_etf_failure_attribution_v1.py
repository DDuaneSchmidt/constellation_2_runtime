...
def main() -> int:
    parser = argparse.ArgumentParser(description="Build Aegis Alpha Factory cross-asset ETF pilot failure attribution V1.")
    parser.add_argument("--truth-root", "--truth_root", dest="truth_root", required=True)
    parser.add_argument("--day", required=True)
    args = parser.parse_args()

    payload = build_alpha_factory_cross_asset_etf_failure_attribution_v1(
    ...
    print("AEGIS ALPHA FACTORY CROSS-ASSET ETF FAILURE ATTRIBUTION v1")
    ...
    return 0 if payload["verdicts"]["execution"] == "CROSS_ASSET_FAILURE_ATTRIBUTION_EXECUTION_VALID" else 1
