...
def main() -> int:
    parser = argparse.ArgumentParser(description="Build Aegis Alpha Factory technical conditional pattern pilot V1.")
    parser.add_argument("--truth-root", "--truth_root", dest="truth_root", required=True)
    parser.add_argument("--day", required=True)
    args = parser.parse_args()

    payload = build_alpha_factory_technical_conditional_pattern_pilot_v1(
        truth_root=Path(args.truth_root),
        day_utc=str(args.day),
    )
    paths = write_alpha_factory_technical_conditional_pattern_pilot_v1(
        truth_root=Path(args.truth_root),
        day_utc=str(args.day),
        payload=payload,
    )
    print("AEGIS ALPHA FACTORY TECHNICAL CONDITIONAL PATTERN PILOT v1")
    ...
    return 0 if payload["verdicts"]["execution"] == "TECHNICAL_CONDITIONAL_PATTERN_PILOT_EXECUTION_VALID" else 1
