...
SCHEMA = "governance/04_DATA/SCHEMAS/C2/REPORTS/safety_state_authority.v1.schema.json"


def _execution_root(raw: str, *, account: str, environment: str) -> Path:
    if str(raw or "").strip():
...
def main(argv: list[str] | None = None) -> int:
    ...
    parser = argparse.ArgumentParser(prog="run_safety_state_authority_v1")
    ...
    account = str(args.account or "").strip() or resolve_single_paper_ib_account_from_sleeve_registry(REPO_ROOT)
    truth_root = resolve_decision_truth_root_bridge_v1(
        args.truth_root or "",
        repo_root=REPO_ROOT,
        caller="ops/tools/run_safety_state_authority_v1.py",
    )
    payload = evaluate_safety_state_authority_v1(
        day_utc=day_utc,
        truth_root=truth_root,
        execution_root=_execution_root(args.execution_root, account=account, environment=environment),
        account=account,
        environment=environment,
    )
    validate_against_repo_schema_v1(payload, REPO_ROOT, SCHEMA)
    path = write_safety_state_authority_v1(truth_root=truth_root, day_utc=day_utc, payload=payload)
    print(
        json.dumps(
            {
