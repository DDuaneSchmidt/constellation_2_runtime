...
raise SystemExit('LEGACY_PROMOTION_GATE_DISABLED_USE_ADVISORY_KERNEL')
...
def _gate_runtime_path(output_root: str, mode: str, day_utc: str) -> Path:
    return advisor_runtime_path(output_root, mode, 'promotion_gate_result_v1', day_utc, 'promotion_gate_result.v1.json')


def _record_runtime_path(output_root: str, mode: str, day_utc: str) -> Path:
    return advisor_runtime_path(output_root, mode, 'promotion_record_v1', day_utc, 'promotion_record.v1.json')
...
def _enforce_authority_registry(*, output_root: str, mode: str, day_utc: str) -> None:
    ...
    if row.authority_class != 'promotion_authority':
...
def main() -> int:
    ap = argparse.ArgumentParser(description='Emit canonical promotion_record.v1.json and compatibility promotion_gate_result.v1.json')
    ...
    gate_result = build_promotion_gate_result(candidate=candidate, review=review, manual_review=manual_review, envelope=gate_env)
    record_env = metadata_envelope_v1(
        produced_utc=args.produced_utc,
        day_utc=args.day_utc,
        mode=args.mode,
        ...
        selection_basis=gate_result.gate_status,
    )
    ...
    record_out = _write(
        _record_runtime_path(args.output_root, args.mode, args.day_utc),
        record.to_dict(),
    )
    gate_out = _write(
        _gate_runtime_path(args.output_root, args.mode, args.day_utc),
        gate_result.to_dict(),
    )
    print(f'OK: {record_out} compat={gate_out}')
    return 0
