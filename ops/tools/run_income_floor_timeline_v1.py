...
from constellation_2.common.advisor_kernel.income_floor_timeline_service import build_income_floor_timeline


def main() -> int:
    parser = argparse.ArgumentParser(description='Emit income_floor_timeline_v1')
    ...
    artifact = build_income_floor_timeline(planning_snapshot=planning, metadata=metadata)
    out = _write(_runtime_path(args.output_root, args.mode, 'income_floor_timeline_v1', args.day_utc, 'income_floor_timeline.v1.json'), artifact.to_dict())
    print(f'OK: {out}')
    return 0
