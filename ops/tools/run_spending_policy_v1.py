...
def main() -> int:
    parser = argparse.ArgumentParser(description='Emit spending_policy_v1')
    ...
    artifact = build_spending_policy(planning_snapshot=planning, metadata=metadata)
    out = _write(_runtime_path(args.output_root, args.mode, 'spending_policy_v1', args.day_utc, 'spending_policy.v1.json'), artifact.to_dict())
    print(f'OK: {out}')
    return 0
