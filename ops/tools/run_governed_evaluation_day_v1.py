...
def main() -> int:
    ap = argparse.ArgumentParser(prog="run_governed_evaluation_day_v1")
    ...
    ap.add_argument("--sleeve_id", action="append", default=[], help="Optional logical sleeve id to evaluate; repeatable")
    args = ap.parse_args()
    ...
    result = materialize_governed_evaluation_day_v1(
        ...
        sleeve_ids=[str(item).strip() for item in args.sleeve_id if str(item).strip()] or None,
    )
