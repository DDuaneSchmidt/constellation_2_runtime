...
from constellation_2.common.advisor_kernel.decision_monitor_service import build_decision_monitor_state


def main() -> int:
    parser = argparse.ArgumentParser(description='Emit decision_monitor_state_v1')
    parser.add_argument('--replan_trigger_rule_json', required=True)
    ...
    decision_plan = _plan(args.decision_plan_json)
    trigger = _trigger(args.replan_trigger_rule_json)
    metadata = _metadata(args, [f'decision_plan_id:{decision_plan.plan_id}', f'replan_trigger_rule_id:{trigger.trigger_id}'])
    artifact = build_decision_monitor_state(decision_plan=decision_plan, replan_trigger_rule=trigger, metadata=metadata)
    out = _write(_runtime_path(args.output_root, args.mode, 'decision_monitor_state_v1', args.day_utc, 'decision_monitor_state.v1.json'), artifact.to_dict())
    print(f'OK: {out}')
    return 0
