#!/usr/bin/env python3
"""
run_pipeline_manifest_v1_compat_from_attempt_v1.py

Compatibility publisher:
- Reads Orchestrator V2 attempt manifest (v2) and emits legacy pipeline_manifest_v1 artifact.
...
- schema validation required (pipeline_manifest.v1.schema.json)

Top-level status mapping (v1 allows OK|FAIL|DEGRADED):
    ...
    try:
        s = str(
            resolve_release_provenance_release_current_first_v1(
                caller="ops/tools/run_pipeline_manifest_v1_compat_from_attempt_v1.py"
            ).get("git_sha")
            or ""
        ).strip()
        if s:
...
def main() -> int:
    ap = argparse.ArgumentParser(prog="run_pipeline_manifest_v1_compat_from_attempt_v1")
    ...
    out_path = (truth / "reports" / "pipeline_manifest_v1" / day / "pipeline_manifest.v1.json").resolve()
    ...
    if out_path.exists():
        ...
        print(f"OK: PIPELINE_MANIFEST_V1_COMPAT_EXISTS_VALID_SKIP day_utc={day} path={out_path} sha256={ex_sha}")
        return 0
    ...
    notes.append("compat_publisher=v1_from_orchestrator_attempt_manifest_v2")
    ...
    recon_day = (truth / "reports/reconciliation_report_v1" / day).resolve()
    op_gate_day = (truth / "reports/operator_daily_gate_v1" / day).resolve()

    base_ok = "OK" if status_top in ("OK", "DEGRADED") else "FAIL"

    stages: List[Dict[str, Any]] = []
    stages.append(_stage_row("INTENTS", base_ok, True, [], intents_day))
    stages.append(_stage_row("PREFLIGHT", base_ok, True, [], preflight_day))
    stages.append(_stage_row("OMS", base_ok, True, [], oms_day))
    stages.append(_stage_row("ALLOCATION", base_ok, True, [], alloc_day))
    stages.append(_stage_row("PHASED_SUBMISSIONS", base_ok, False, [], phased_root))
    stages.append(_stage_row("EXEC_EVIDENCE_TRUTH", base_ok, True, [], exec_truth_day))
    stages.append(_stage_row("EXEC_EVIDENCE_MANIFEST", base_ok, True, [], exec_manifest_day))
    stages.append(_stage_row("SUBMISSION_INDEX", base_ok, False, [], sub_index_day))
    stages.append(_stage_row("POSITIONS", base_ok, True, [], positions_day))
    stages.append(_stage_row("CASH_LEDGER", base_ok, True, [], cash_day))
    stages.append(_stage_row("RECONCILIATION", base_ok, True, [], recon_day))
    stages.append(_stage_row("OPERATOR_GATE", base_ok, True, [], op_gate_day))

    emitted = [x.get("stage_id") for x in stages]
    if any(sid not in STAGE_IDS_V1 for sid in emitted):
    ...
    out = {
        ...
        "producer": {
            ...
            "module": "ops/tools/run_pipeline_manifest_v1_compat_from_attempt_v1.py",
            "git_sha": _git_sha(),
        },
        "status": status_top,
        "reason_codes": list(reason_codes),
    ...
    print(
        f"OK: PIPELINE_MANIFEST_V1_COMPAT_WRITTEN day_utc={day} status={status_top} "
