...
DEFAULT_OUTPUT_DIR = REPO_ROOT / "docs/aegis/technical_strategy_factory/evidence/backtest_002"
DEFAULT_TIINGO_CACHE_PATH = REPO_ROOT / "data/cache/SPY_tiingo_adjusted_daily.csv"
SCRIPT_PATH = "ops/tools/run_backtest_002_imp_002.py"
...
def append_failure_log(output_dir: Path, *, failure_type: str, failure_message: str, data_source_attempted: str) -> Path:
    ...
    path = output_dir / "backtest_002_failure_log.jsonl"
    entry = {
...
def validate_daily_data(data: pd.DataFrame, *, source: str, requested_start: str) -> dict[str, Any]:
    ...
    if rows_before_first_signal < 200:
        raise BacktestDataError(
            "DATA_VALIDATION_FAILED",
            f"{source} has only {rows_before_first_signal} trading rows before first possible 52-week breakout signal",
            source,
        )
    return {
...
def fetch_tiingo_data(start: str, ticker: str, *, cache_path: Path = DEFAULT_TIINGO_CACHE_PATH) -> tuple[pd.DataFrame, dict[str, Any]]:
    ...
    if not api_key:
        raise BacktestDataError(
            "DATA_SOURCE_FAILURE",
            "TIINGO_API_KEY is missing; Tiingo is the primary BACKTEST_002 data source and keys must not be hardcoded.",
            "tiingo",
        )
...
def run_strategy(
    ...
    *,
    ...
    close = data["Close"]
    open_ = data["Open"]
    prior_high = close.rolling(breakout_window, min_periods=breakout_window).max().shift(1)
    exit_average = close.rolling(exit_window, min_periods=exit_window).mean()
    ...
    for i, dt in enumerate(dates):
        ...
        entry_sig = bool(entry_signal.iloc[i]) if not pd.isna(entry_signal.iloc[i]) else False
        exit_sig = bool(exit_signal.iloc[i]) if not pd.isna(exit_signal.iloc[i]) else False
        if in_market and exit_sig and not any(item["action"] == "exit" for item in pending):
        ...
        elif (not in_market) and entry_sig and not any(item["action"] == "entry" for item in pending):
            execute_index = i + execution_delay_sessions
            if execute_index < len(dates):
...
def random_baseline(
    ...
    *,
    ...
    return {
        ...
        "method": "random daily invested masks with exactly the same rounded invested-day count as IMP_002; 10 bps round-trip costs applied on random exposure transitions",
        ...
        "imp_002_percentile_rank_cagr": as_float(percentile_rank(imp_metrics.get("cagr"), "cagr")),
        "imp_002_percentile_rank_sharpe": as_float(percentile_rank(imp_metrics.get("sharpe"), "sharpe")),
        "imp_002_percentile_rank_max_drawdown": as_float(percentile_rank(imp_metrics.get("maximum_drawdown"), "maximum_drawdown", higher_is_better=False)),
    }
...
def random_table(random_result: dict[str, Any]) -> str:
    keys = [
        ...
        "p05_random_sharpe",
        "p95_random_sharpe",
        "imp_002_percentile_rank_cagr",
        "imp_002_percentile_rank_sharpe",
        "imp_002_percentile_rank_max_drawdown",
    ]
    lines = ["| Metric | Value |", "|---|---|"]
    for key in keys:
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Run BACKTEST_002 for IMP_002 SPY 52-Week Breakout.")
    ...
    imp = run_strategy(data, name="IMP_002", breakout_window=252, exit_window=100, cost_bps_round_trip=10.0, execution_delay_sessions=1)
    ...
    metrics = {
        "IMP_002": metrics_from_equity(imp.equity, position=imp.position, trades=imp.trades),
    ...
    random_result = random_baseline(
        data,
        target_time_in_market=float(metrics["IMP_002"]["time_in_market"]),
        imp_metrics=metrics["IMP_002"],
    ...
    for period_name, (start, end) in periods.items():
        subperiods[period_name] = {
            "IMP_002": slice_result(imp, start, end),
    ...
    stress_results = {
        "20_bps_round_trip_cost": run_strategy(data, name="20_bps_round_trip_cost", breakout_window=252, exit_window=100, cost_bps_round_trip=20.0, execution_delay_sessions=1),
        "50_bps_round_trip_cost": run_strategy(data, name="50_bps_round_trip_cost", breakout_window=252, exit_window=100, cost_bps_round_trip=50.0, execution_delay_sessions=1),
        "one_day_execution_delay": run_strategy(data, name="one_day_execution_delay", breakout_window=252, exit_window=100, cost_bps_round_trip=10.0, execution_delay_sessions=2),
        "200_day_breakout_perturbation": run_strategy(data, name="200_day_breakout_perturbation", breakout_window=200, exit_window=100, cost_bps_round_trip=10.0, execution_delay_sessions=1),
        "300_day_breakout_perturbation": run_strategy(data, name="300_day_breakout_perturbation", breakout_window=300, exit_window=100, cost_bps_round_trip=10.0, execution_delay_sessions=1),
        "50dma_exit_perturbation": run_strategy(data, name="50dma_exit_perturbation", breakout_window=252, exit_window=50, cost_bps_round_trip=10.0, execution_delay_sessions=1),
        "150dma_exit_perturbation": run_strategy(data, name="150dma_exit_perturbation", breakout_window=252, exit_window=150, cost_bps_round_trip=10.0, execution_delay_sessions=1),
    }
    stress_tests = {
    ...
    manifest_paths = {
        "summary": str(output_dir / "backtest_002_summary.json"),
        "metrics": str(output_dir / "backtest_002_metrics.json"),
        "random_baseline": str(output_dir / "backtest_002_random_baseline.json"),
        "subperiods": str(output_dir / "backtest_002_subperiods.json"),
        "stress_tests": str(output_dir / "backtest_002_stress_tests.json"),
        "run_manifest": str(output_dir / "backtest_002_run_manifest.json"),
        "evidence_report": str(output_dir / "backtest_002_evidence_report.md"),
    }
    ...
    manifest = {
        ...
        "execution_policy": "signal_after_close_execute_next_market_open_no_same_day_execution",
    ...
    output_dir.mkdir(parents=True, exist_ok=True)
    write_json(output_dir / "backtest_002_summary.json", summary)
    write_json(output_dir / "backtest_002_metrics.json", {"metrics": metrics})
    write_json(output_dir / "backtest_002_random_baseline.json", random_result)
    write_json(output_dir / "backtest_002_subperiods.json", {"subperiods": subperiods})
    write_json(output_dir / "backtest_002_stress_tests.json", {"stress_tests": stress_tests})
    write_json(output_dir / "backtest_002_run_manifest.json", manifest)
    (output_dir / "backtest_002_evidence_report.md").write_text(
        build_report(
            metadata=metadata,
            summary=summary,
            metrics=metrics,
