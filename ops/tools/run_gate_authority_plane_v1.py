...
def main() -> int:
    ...
    print(
        ...
        f"lifecycle_state_action={writes['lifecycle_state'].action} "
        f"authorization_verdict_action={writes['authorization_verdict'].action} "
        f"economic_verdict_action={writes['economic_verdict'].action} "
        f"ledger_action={writes['ledger'].action}"
    )
    return 0
