...
Enforces Bundle A:
  ...
  - No SIMULATED submissions
  - No SYNTH_ markers in execution evidence subtree
...
def main() -> int:
    ...
    # Broker truth manifest must be OK if submissions exist (institutional boundary).
    broker_ok = True
    ...
    verdict["verdict_sha256"] = _compute_self_sha_field(verdict, "verdict_sha256")

    _validate_jsonschema_or_fail(verdict, schema_path)

    out_dir = (OUT_ROOT / day).resolve()
    out_path = (out_dir / "operator_gate_verdict.v1.json").resolve()
    wr = _write_immutable_canonical_json(out_path, verdict)
