...
from constellation_2.common.advisor_bridge.promotion_plane_service import build_promotion_review
...
def _runtime_path(output_root: str, mode: str, day_utc: str) -> Path:
    return advisor_runtime_path(output_root, mode, 'promotion_review_v1', day_utc, 'promotion_review.v1.json')

def main() -> int:
    ap = argparse.ArgumentParser(description='Emit promotion_review.v1.json')
    ...
    args = ap.parse_args()
    candidate = PromotionCandidateV1.load_file(args.promotion_candidate_json)
    env = metadata_envelope_v1(produced_utc=args.produced_utc, day_utc=args.day_utc, mode=args.mode, source_artifact_refs=[f'promotion_candidate_id:{candidate.candidate_id}'], artifact_family='promotion_review_v1')
    out = _write(_runtime_path(args.output_root, args.mode, args.day_utc), build_promotion_review(candidate=candidate, envelope=env).to_dict())
    print(f'OK: {out}')
    return 0
