...
def main() -> int:
    ap = argparse.ArgumentParser(prog="run_batch2_visibility_v1")
    ...
    print(
        "OK: BATCH2_VISIBILITY_V1 "
        f"day_utc={day_utc} truth_root={truth_root} "
        f"snapshot_action={writes['snapshot'].action} "
        f"funnel_action={writes['funnel'].action} "
        f"drift_action={writes['drift'].action} "
        f"summary_action={writes['summary'].action} "
