...
def _runtime_path(output_root: str, mode: str, day_utc: str) -> Path:
    return advisor_runtime_path(output_root, mode, 'promotion_manual_review_v1', day_utc, 'promotion_manual_review.v1.json')


def main() -> int:
    ap = argparse.ArgumentParser(description='Emit promotion_manual_review.v1.json')
    ap.add_argument('--promotion_candidate_json', required=True)
    ap.add_argument('--promotion_review_json', required=True)
    ap.add_argument('--operator_id', required=True)
    ap.add_argument('--manual_review_status', required=True)
    ap.add_argument('--operator_notes', required=True)
    ...
    env = metadata_envelope_v1(
        ...
        mode=args.mode,
        source_artifact_refs=[f'promotion_candidate_id:{candidate.candidate_id}', f'promotion_review_id:{review.review_id}'],
        artifact_family='promotion_manual_review_v1',
        selection_basis=args.manual_review_status,
    )
    out = _write(
