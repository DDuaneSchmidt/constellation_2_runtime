...
from constellation_2.common.advisor_kernel.readiness_service import build_readiness_outputs


def main() -> int:
    parser = argparse.ArgumentParser(description='Emit readiness artifacts')
    ...
    metadata = _metadata(args, [f'planning_snapshot_id:{planning.planning_snapshot_id}', f'official_recommendation_set_id:{official.advisory_packet_id}'])
    score, missing, complete = build_readiness_outputs(planning_snapshot=planning, official_recommendation_set=official, metadata=metadata)
    _write(_runtime_path(args.output_root, args.mode, 'domain_readiness_score_v1', args.day_utc, 'domain_readiness_score.v1.json'), score.to_dict())
    _write(_runtime_path(args.output_root, args.mode, 'missing_input_priority_v1', args.day_utc, 'missing_input_priority.v1.json'), missing.to_dict())
    out = _write(_runtime_path(args.output_root, args.mode, 'advice_completeness_report_v1', args.day_utc, 'advice_completeness_report.v1.json'), complete.to_dict())
    print(f'OK: {out}')
    return 0
