...
from constellation_2.common.advisor_kernel.tax_budget_service import build_tax_budget


def main() -> int:
    parser = argparse.ArgumentParser(description='Emit tax_budget_v1')
    ...
    artifact = build_tax_budget(planning_snapshot=planning, metadata=metadata)
    out = _write(_runtime_path(args.output_root, args.mode, 'tax_budget_v1', args.day_utc, 'tax_budget.v1.json'), artifact.to_dict())
    print(f'OK: {out}')
    return 0
