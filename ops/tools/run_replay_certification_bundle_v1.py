...
Writes a formal replay certification artifact (immutable, day-keyed):
  truth/reports/replay_certification_bundle_v1/<DAY>/replay_certification_bundle.v1.json

Fail-closed semantics:
- If any required input artifact is missing, status=FAIL.
- Bundle is still written immutably (so that failures are auditable), but downstream LIVE advancement must require PASS via gate.

Hash tree digest:
- sha256 over sorted lines "sha256  path" (repo-relative) for all inputs listed in inputs[] where present=true.
"""
...
def main() -> int:
    ...
    # canonical input locations (no placeholders)
    ...
    missing_types = [e["type"] for e in inputs if not e["present"]]
    fail_closed = bool(missing_types)
    status = "FAIL" if fail_closed else "PASS"

    # hashes required by schema (use zeros if missing, but FAIL will be set)
    ...
    # sha256 tree digest over present inputs
    lines: List[str] = []
    for e in inputs:
        if e["present"]:
