...
DEFAULT_OUTPUT_DIR = REPO_ROOT / "docs/aegis/technical_strategy_factory/evidence/backtest_003"
DEFAULT_TIINGO_CACHE_PATH = REPO_ROOT / "data/cache/SPY_tiingo_adjusted_daily.csv"
SCRIPT_PATH = "ops/tools/run_backtest_003_imp_003.py"
...
def append_failure_log(output_dir: Path, *, failure_type: str, failure_message: str, data_source_attempted: str) -> Path:
    ...
    path = output_dir / "backtest_003_failure_log.jsonl"
    entry = {
...
def validate_daily_data(data: pd.DataFrame, *, source: str, requested_start: str) -> dict[str, Any]:
    ...
    if rows_before_first_signal < 200:
        raise BacktestDataError(
            "DATA_VALIDATION_FAILED",
            f"{source} has only {rows_before_first_signal} trading rows before first possible Turtle breakout signal",
            source,
        )
    return {
...
def fetch_tiingo_data(start: str, ticker: str, *, cache_path: Path = DEFAULT_TIINGO_CACHE_PATH) -> tuple[pd.DataFrame, dict[str, Any]]:
    ...
    if not api_key:
        raise BacktestDataError(
            "DATA_SOURCE_FAILURE",
            "TIINGO_API_KEY is missing; Tiingo is the primary BACKTEST_003 data source and keys must not be hardcoded.",
            "tiingo",
        )
...
def random_baseline(
    ...
    *,
    ...
    return {
        ...
        "method": "random daily invested masks with exactly the same rounded invested-day count as IMP_003; 10 bps round-trip costs applied on random exposure transitions",
        ...
        "imp_003_percentile_rank_cagr": as_float(percentile_rank(imp_metrics.get("cagr"), "cagr")),
        "imp_003_percentile_rank_sharpe": as_float(percentile_rank(imp_metrics.get("sharpe"), "sharpe")),
        "imp_003_percentile_rank_max_drawdown": as_float(percentile_rank(imp_metrics.get("maximum_drawdown"), "maximum_drawdown", higher_is_better=False)),
    }
...
def random_table(random_result: dict[str, Any]) -> str:
    keys = [
        ...
        "p05_random_sharpe",
        "p95_random_sharpe",
        "imp_003_percentile_rank_cagr",
        "imp_003_percentile_rank_sharpe",
        "imp_003_percentile_rank_max_drawdown",
    ]
    lines = ["| Metric | Value |", "|---|---|"]
    for key in keys:
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Run BACKTEST_003 for IMP_003 SPY Turtle Breakout.")
    ...
    imp = run_strategy(data, name="IMP_003", breakout_window=55, exit_window=20, cost_bps_round_trip=10.0, execution_delay_sessions=1)
    ...
    metrics = {
        "IMP_003": metrics_from_equity(imp.equity, position=imp.position, trades=imp.trades),
    ...
    random_result = random_baseline(
        data,
        target_time_in_market=float(metrics["IMP_003"]["time_in_market"]),
        imp_metrics=metrics["IMP_003"],
    ...
    for period_name, (start, end) in periods.items():
        subperiods[period_name] = {
            "IMP_003": slice_result(imp, start, end),
    ...
    stress_results = {
        "20_bps_round_trip_cost": run_strategy(data, name="20_bps_round_trip_cost", breakout_window=55, exit_window=20, cost_bps_round_trip=20.0, execution_delay_sessions=1),
        "50_bps_round_trip_cost": run_strategy(data, name="50_bps_round_trip_cost", breakout_window=55, exit_window=20, cost_bps_round_trip=50.0, execution_delay_sessions=1),
        "one_day_execution_delay": run_strategy(data, name="one_day_execution_delay", breakout_window=55, exit_window=20, cost_bps_round_trip=10.0, execution_delay_sessions=2),
        "40_day_breakout_perturbation": run_strategy(data, name="40_day_breakout_perturbation", breakout_window=40, exit_window=20, cost_bps_round_trip=10.0, execution_delay_sessions=1),
        "70_day_breakout_perturbation": run_strategy(data, name="70_day_breakout_perturbation", breakout_window=70, exit_window=20, cost_bps_round_trip=10.0, execution_delay_sessions=1),
        "10_day_low_exit_perturbation": run_strategy(data, name="10_day_low_exit_perturbation", breakout_window=55, exit_window=10, cost_bps_round_trip=10.0, execution_delay_sessions=1),
        "30_day_low_exit_perturbation": run_strategy(data, name="30_day_low_exit_perturbation", breakout_window=55, exit_window=30, cost_bps_round_trip=10.0, execution_delay_sessions=1),
    }
    stress_tests = {
    ...
    manifest_paths = {
        "summary": str(output_dir / "backtest_003_summary.json"),
        "metrics": str(output_dir / "backtest_003_metrics.json"),
        "random_baseline": str(output_dir / "backtest_003_random_baseline.json"),
        "subperiods": str(output_dir / "backtest_003_subperiods.json"),
        "stress_tests": str(output_dir / "backtest_003_stress_tests.json"),
        "run_manifest": str(output_dir / "backtest_003_run_manifest.json"),
        "evidence_report": str(output_dir / "backtest_003_evidence_report.md"),
    }
    ...
    output_dir.mkdir(parents=True, exist_ok=True)
    write_json(output_dir / "backtest_003_summary.json", summary)
    write_json(output_dir / "backtest_003_metrics.json", {"metrics": metrics})
    write_json(output_dir / "backtest_003_random_baseline.json", random_result)
    write_json(output_dir / "backtest_003_subperiods.json", {"subperiods": subperiods})
    write_json(output_dir / "backtest_003_stress_tests.json", {"stress_tests": stress_tests})
    write_json(output_dir / "backtest_003_run_manifest.json", manifest)
    (output_dir / "backtest_003_evidence_report.md").write_text(
        build_report(
            metadata=metadata,
            summary=summary,
            metrics=metrics,
