...
from ops.aegis.account_economic_state_authority_v1 import build_and_write_account_economic_state_authority_v1


def main() -> int:
    parser = argparse.ArgumentParser(prog="build_account_economic_state_authority_v1")
    ...
    result = build_and_write_account_economic_state_authority_v1(
        truth_root=Path(args.truth_root),
        day_utc=str(args.day_utc),
        operation_type=str(args.operation_type),
        sleeve_id=str(args.sleeve_id),
        environment=str(args.environment),
        ib_account=str(args.ib_account),
        emit_events=not bool(args.no_events),
    )
