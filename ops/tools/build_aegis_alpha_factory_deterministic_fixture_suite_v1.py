...
def main() -> int:
    parser = argparse.ArgumentParser(description="Build Aegis Alpha Factory deterministic fixture suite V1.")
    parser.add_argument("--truth-root", "--truth_root", dest="truth_root", required=True)
    parser.add_argument("--day", required=True)
    args = parser.parse_args()
    payload = build_alpha_factory_deterministic_fixture_suite_v1(
        truth_root=Path(args.truth_root),
        day_utc=str(args.day),
    )
    paths = write_alpha_factory_deterministic_fixture_suite_v1(
        truth_root=Path(args.truth_root),
        day_utc=str(args.day),
        payload=payload,
    )
    print("AEGIS ALPHA FACTORY DETERMINISTIC FIXTURE SUITE v1")
