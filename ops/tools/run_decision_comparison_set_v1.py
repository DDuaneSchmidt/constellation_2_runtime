...
from constellation_2.common.advisor_kernel.recommendation_comparison_service import build_decision_comparison_set


def main() -> int:
    parser = argparse.ArgumentParser(description='Emit decision_comparison_set_v1')
    ...
    artifact = build_decision_comparison_set(planning_snapshot=planning, official_recommendation_set=official, metadata=metadata)
    out = _write(_runtime_path(args.output_root, args.mode, 'decision_comparison_set_v1', args.day_utc, 'decision_comparison_set.v1.json'), artifact.to_dict())
    print(f'OK: {out}')
    return 0
