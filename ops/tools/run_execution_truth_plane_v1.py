...
from constellation_2.common.execution_truth_foundation_v1 import (  # noqa: E402
    _require_day_utc,
    _require_produced_utc,
    write_execution_truth_plane,
)
...
def _resolve_truth_root(raw: str) -> Path:
    ...
    truth_root = Path(text).expanduser().resolve() if text else resolve_truth_root(repo_root=REPO_ROOT)
...
def main() -> int:
    ap = argparse.ArgumentParser(prog="run_execution_truth_plane_v1")
    ...
    writes = write_execution_truth_plane(
        repo_root=REPO_ROOT,
    ...
    print(
        "OK: EXECUTION_TRUTH_PLANE_V1 "
        f"day_utc={day_utc} truth_root={truth_root} "
        f"normalization_action={writes['normalization'].action} "
        f"lifecycle_action={writes['lifecycle'].action} "
        f"economic_action={writes['economic'].action} "
        f"gap_action={writes['gap'].action}"
    )
    return 0
