...
def main() -> int:
    parser = argparse.ArgumentParser(description='Invoke canonical advisor_execution decision plan delta path')
    ...
    parser.add_argument('--prior_decision_plan_json', required=True)
    parser.add_argument('--current_decision_plan_json', required=True)
    ...
    delta = canonical_dir / 'decision_plan_delta.v1.json'
    _call('run_decision_plan_delta_v1.py', ['--previous_plan_json', args.prior_decision_plan_json, '--current_plan_json', args.current_decision_plan_json, '--out_json', str(delta)])
    print(f'OK: {delta}')
    return 0
