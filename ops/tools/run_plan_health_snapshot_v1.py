...
from constellation_2.common.advisor_kernel.plan_health_service import build_plan_health_snapshot
from constellation_2.common.advisor_kernel.semantic_reconciliation_report_v1 import SemanticReconciliationReportV1


def main() -> int:
    parser = argparse.ArgumentParser(description='Emit plan_health_snapshot_v1')
    ...
    metadata = _metadata(args, [f'decision_plan_id:{decision_plan.plan_id}', f'semantic_report_id:{semantic.run_id}'])
    artifact = build_plan_health_snapshot(decision_plan=decision_plan, semantic_report=semantic, metadata=metadata)
    out = _write(_runtime_path(args.output_root, args.mode, 'plan_health_snapshot_v1', args.day_utc, 'plan_health_snapshot.v1.json'), artifact.to_dict())
    print(f'OK: {out}')
    return 0
