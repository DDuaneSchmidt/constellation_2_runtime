...
from constellation_2.common.advisor_bridge.advisor_trade_bridge_service import build_advisor_trade_translation
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Emit advisor_trade_translation.v1.json")
    ...
    artifact = build_advisor_trade_translation(decision_plan=decision_plan, planning_snapshot=planning_snapshot, metadata=metadata)
    out = _write(_runtime_path(args.output_root, args.mode, "advisor_trade_translation_v1", args.day_utc, "advisor_trade_translation.v1.json"), artifact.to_dict())
    print(f"OK: {out}")
    return 0
