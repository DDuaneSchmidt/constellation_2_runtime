...
from constellation_2.common.execution_mode_authority_v1 import (
    evaluate_execution_mode_authority_v1,
    write_execution_mode_authority_v1,
)
...
SCHEMA = "governance/04_DATA/SCHEMAS/C2/REPORTS/execution_mode_authority.v1.schema.json"
...
def main(argv: list[str] | None = None) -> int:
    ...
    parser = argparse.ArgumentParser(prog="run_execution_mode_authority_v1")
    ...
    truth_root = resolve_decision_truth_root_bridge_v1(
        args.truth_root or "",
        repo_root=REPO_ROOT,
        caller="ops/tools/run_execution_mode_authority_v1.py",
    )
    ...
    output_path = write_execution_mode_authority_v1(truth_root=truth_root, day_utc=day_utc, payload=payload)
    print(
        json.dumps(
            {
                "status": payload["status"],
                "mode_state": payload["mode_state"],
                "broker_transmit_enabled": payload["broker_transmit_enabled"],
                "missing_broker_ids_blocker": payload["missing_broker_ids_blocker"],
