...
def main() -> int:
    parser = argparse.ArgumentParser(description='Emit semantic_reconciliation_report_v1')
    ...
    metadata = _metadata(args, [f'planning_snapshot_id:{planning.planning_snapshot_id}', f'official_recommendation_set_id:{official.advisory_packet_id}', f'decision_plan_id:{plan.plan_id}'])
    artifact = build_semantic_reconciliation_report(planning_snapshot=planning, decision_plan=plan, metadata=metadata)
    out = _write(_runtime_path(args.output_root, args.mode, 'semantic_reconciliation_report_v1', args.day_utc, 'semantic_reconciliation_report.v1.json'), artifact.to_dict())
    print(f'OK: {out}')
    return 0
