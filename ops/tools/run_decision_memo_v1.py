...
from constellation_2.common.advisor_kernel.decision_memo_service import build_decision_memo
...
def main() -> int:
    parser = argparse.ArgumentParser(description='Emit why_not_reason_v1, alternative_rejection_ledger_v1, decision_memo_v1')
    parser.add_argument('--decision_plan_json', required=True)
    parser.add_argument('--semantic_report_json', required=True)
    parser.add_argument('--publication_gate_json', required=True)
    parser.add_argument('--official_recommendation_set_json', required=True)
    parser.add_argument('--blocked_actions_json', required=True)
    parser.add_argument('--mode', required=True)
    parser.add_argument('--day_utc', required=True)
    parser.add_argument('--produced_utc', required=True)
    parser.add_argument('--output_root', required=True)
    args = parser.parse_args()
    plan = _plan(args.decision_plan_json)
    semantic = SemanticReconciliationReportV1.load_file(args.semantic_report_json)
    gates = _publication_gate_results(args.publication_gate_json, PublicationGateResultV1)
    official = _official(args.official_recommendation_set_json)
    blocked = _blocked(args.blocked_actions_json)
    metadata = _metadata(args, [f'decision_plan_id:{plan.plan_id}', f'semantic_report_id:{semantic.run_id}', f'official_recommendation_set_id:{official.advisory_packet_id}'] + [f'blocked_action_id:{item.action_id}' for item in blocked] + [f'publication_gate_result_id:{item.run_id}' for item in gates])
    why_not, ledger, memo = build_decision_memo(decision_plan=plan, semantic_reconciliation_report=semantic, publication_gate_results=gates, official_recommendation_set=official, blocked_actions=blocked, metadata=metadata)
    _write(_runtime_path(args.output_root, args.mode, 'why_not_reason_v1', args.day_utc, 'why_not_reason.v1.json'), why_not.to_dict())
    _write(_runtime_path(args.output_root, args.mode, 'alternative_rejection_ledger_v1', args.day_utc, 'alternative_rejection_ledger.v1.json'), ledger.to_dict())
    out = _write(_runtime_path(args.output_root, args.mode, 'decision_memo_v1', args.day_utc, 'decision_memo.v1.json'), memo.to_dict())
    print(f'OK: {out}')
    return 0
