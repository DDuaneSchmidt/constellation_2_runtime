...
TRUTH = resolve_truth_root_bridge_v1(repo_root=REPO_ROOT, caller="ops/tools/run_systemic_risk_gate_v3.py")
...
def _eval_sentinel_and_override(day: str) -> Tuple[bool, List[str], List[Dict[str, str]]]:
    ...
    sentinel_path = (PATH_SENTINEL / day / "stress_drift_sentinel.v1.json").resolve()
    if not sentinel_path.exists():
        manifest.append({"type": "stress_drift_sentinel_v1_missing", "path": str(sentinel_path), "sha256": _sha256_bytes(b"")})
        rc.append("MISSING_STRESS_DRIFT_SENTINEL_V1")
        rc.append("STRESS_OVERRIDE_REQUIRED")
        return (False, rc, manifest)

    manifest.append({"type": "stress_drift_sentinel_v1", "path": str(sentinel_path), "sha256": _sha256_file(sentinel_path)})

    try:
        s = _read_json_obj(sentinel_path)
    except Exception:
        rc.append("STRESS_DRIFT_SENTINEL_PARSE_ERROR")
        rc.append("STRESS_OVERRIDE_REQUIRED")
        return (False, rc, manifest)

    esc = bool(s.get("escalation_recommended"))
    if not esc:
    ...
    manifest.append({"type": "operator_stress_override_v1", "path": str(override_path), "sha256": _sha256_file(override_path)})
