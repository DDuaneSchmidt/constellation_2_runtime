...
from constellation_2.common.trading_day_closure_authority_v1 import (
    evaluate_trading_day_closure_authority_v1,
    write_trading_day_closure_authority_v1,
)
from constellation_2.phaseD.lib.validate_against_schema_v1 import validate_against_repo_schema_v1
from ops.tools.c2_account_resolution_v1 import resolve_single_paper_ib_account_from_sleeve_registry


SCHEMA = "governance/04_DATA/SCHEMAS/C2/REPORTS/trading_day_closure_authority.v1.schema.json"


def _resolve_execution_root(raw: str) -> Path:
...
def main(argv: list[str] | None = None) -> int:
    ...
    parser = argparse.ArgumentParser(prog="run_trading_day_closure_authority_v1")
    ...
    truth_root = resolve_decision_truth_root_bridge_v1(
        args.truth_root or "",
        repo_root=REPO_ROOT,
        caller="ops/tools/run_trading_day_closure_authority_v1.py",
    )
    execution_root = _resolve_execution_root(str(args.execution_root))
    payload = evaluate_trading_day_closure_authority_v1(day_utc=day_utc, truth_root=truth_root, execution_root=execution_root)
    validate_against_repo_schema_v1(payload, REPO_ROOT, SCHEMA)
    output_path = write_trading_day_closure_authority_v1(truth_root=truth_root, day_utc=day_utc, payload=payload)
    print(
        json.dumps(
            {
