...
- reports/systemic_risk_gate_v1/<DAY>/systemic_risk_gate.v1.json
...
- correlation matrix status DEGRADED_INSUFFICIENT_HISTORY is acceptable ONLY when matrix is 1x1 (no pairwise risk possible).
  This preserves fail-closed behavior while allowing first-day bootstrap.
"""
...
TRUTH = resolve_canonical_truth_root_bridge_v1(caller="ops/tools/run_systemic_risk_gate_v1.py").resolve()

SCHEMA_RELPATH = "governance/04_DATA/SCHEMAS/C2/RISK/systemic_risk_gate.v1.schema.json"
OUT_ROOT = (TRUTH / "reports" / "systemic_risk_gate_v1").resolve()
...
def _eval_corr(day: str, threshold: str) -> Tuple[bool, Dict[str, Any], Dict[str, Any], List[str], List[Dict[str, str]]]:
    ...
    # compute max off-diagonal abs correlation
    max_pair = Decimal("0")
    ...
    # If threshold breach: always fail-closed.
    if abs(max_pair) >= thr:
        rc.append("CORRELATION_THRESHOLD_BREACH")
        return (False, o, shock, rc, manifest)

    # Bootstrap acceptance: DEGRADED is OK only when no pairwise risk exists (1x1).
...
def main() -> int:
    ap = argparse.ArgumentParser(prog="run_systemic_risk_gate_v1")
    ...
    payload: Dict[str, Any] = {
        ...
        "producer": {"repo": "constellation_2_runtime", "module": "ops/tools/run_systemic_risk_gate_v1.py", "git_sha": _git_sha()},
        "status": status,
    ...
    out_path = (OUT_ROOT / day / "systemic_risk_gate.v1.json").resolve()
    try:
    ...
    print(f"OK: SYSTEMIC_RISK_GATE_V1_WRITTEN day_utc={day} status={status} path={wr.path} sha256={wr.sha256} action={wr.action}")
