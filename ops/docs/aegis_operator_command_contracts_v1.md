...
If the symlink advances but `current_release.v1.json` remains stale, the launcher will run the old release. Treat that as a blocker and repair only the current-release pointer after verifying the activated release manifest hash.
