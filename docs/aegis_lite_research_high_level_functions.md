...
| 8 | Records manual execution receipts, fills, slippage, stop behavior, and outcome attribution. | `NEEDS_REWORDING` | `READY_WITH_MANUAL_STEPS` | Schemas/builders and CLIs support manual receipts, fill details, slippage, stop status, outcome ledgers, and attribution. The operator must provide the evidence; no IB fill import is active. |
...
8. Trade capture alert gate/ledger/message bodies exist; actual SMS/email delivery is not proven and should be labeled `DRY_RUN_MESSAGE_BODY_ONLY`.
9. The operator manually enters any IB paper trade and immediately records stop/receipt evidence with `record_manual_execution_receipt_v1.py`.
10. Aegis can measure recommendation vs manual receipt vs outcome when the operator supplies receipt/outcome data with `record_trade_outcome_v1.py`.
...
- `sleeve_performance_report.v1` offline report and print-summary CLI.
- Manual receipt and trade outcome entry CLIs.
- Minimal fee-adjusted advisor benchmark artifact/CLI.
...
- Advisor benchmark comparison UI/report integration.
...
- Fee-adjusted advisor benchmark UI/report integration.
...
- Do not claim advisor benchmark comparison is part of the Lite UI or sleeve performance report yet; only the standalone CLI/artifact is implemented.
